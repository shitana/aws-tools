...
usage() {
    echo "Usage: $0 -k <key_word> [-e <environment>] [-c] [-i <ip_address>]"
    echo "Options :"
    echo "  -k <key_word> : Specify the keyword to search"
    echo "  -e <environment> : Specify the environment (dev, prod, prep, pfi)"
...
case $ENVIRONMENT in
    dev)
        ...
        ;;
    prod)
        AWS_PROFILE="profile_prod"
        ;;
    prep)
        AWS_PROFILE="profile_prep"
        ;;
    *)
...
output=$(aws --region eu-west-1 --profile ${AWS_PROFILE} ec2 describe-instances --filters "Name=instance-state-name,Values=running" "${FILTERS}" --query "Reservations[*].Instances[*].{InstanceId:InstanceId, Name:Tags[?Key=='Name'].Value | [0], PrivateIpAddress:PrivateIpAddress}" | jq -r '.[][] | [.InstanceId, .Name, .PrivateIpAddress] | @tsv')
...
if $DISPLAY_COMMAND; then
    while IFS=$'\t' read -r INSTANCE_ID INSTANCE_NAME INSTANCE_IP; do
        echo "aws ssm start-session --profile ${AWS_PROFILE} --target $INSTANCE_ID #$INSTANCE_NAME"
    done <<< "$output"
fi
